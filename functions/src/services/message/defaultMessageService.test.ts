...
describe('DefaultMessageService', () => {
  ...
  describe('registerDevice', () => {
    it('should create a new device document when registering a new device', async () => {
      ...
      const devicesQuerySnapshot = await collectionsService.userDevices(userId).get()
  ...
  describe('unregisterDevice', () => {
    it('should remove a device when unregistering', async () => {
      ...
      let devicesQuerySnapshot = await collectionsService.userDevices(userId).get()
      expect(devicesQuerySnapshot.docs.length).to.equal(1)
      
      // Unregister the device
      await messageService.unregisterDevice(userId, token, platform as any)
    ...
    it('should do nothing if the device does not exist', async () => {
      ...
      await messageService.unregisterDevice('nonExistentUser', 'non-existent-token', 'iOS' as any)
  ...
  describe('addMessage', () => {
    it('should add a new message', async () => {
      ...
      const messagesSnapshot = await collectionsService.userMessages(userId).get()
    ...
    it('should handle replaceDuplicates option', async () => {
      ...
      expect(dateValue).to.be.greaterThan(
        message1.creationDate.getTime()
      )
    })
  })
