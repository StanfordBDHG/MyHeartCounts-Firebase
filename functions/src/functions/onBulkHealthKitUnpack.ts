 ...
 */
async function processZlibFile(userId: string, filePath: string, storage: any) {
  try {
    ...
    for (const [key, value] of Object.entries(jsonContent)) {
      ...
      logger.info(`Successfully decompressed and uploaded ${key} from ${filePath}`)
    }
    ...
    logger.error(`Error processing zlib file ${filePath}: ${error}`)
    throw error
  }
}
...
async function processAllZlibFiles() {
  try {
    ...
    const [userDirs] = await bucket.getFiles({ prefix: 'users/', delimiter: '/' })
    ...
    logger.error(`Error processing zlib files: ${error}`)
    throw error
  }
}
...
export const onBulkHealthKitUploaded = onObjectFinalized(
  ...
  async (event) => {
    ...
    if (!filePath.includes('/bulkHealthKitUploads/') || !filePath.endsWith('.zlib')) {
      return
    }
