...
- MHC collects the following data:
  - HealthKit samples:
    - all sample types listed in the study bundle
    - all clinical record types
  - user-provided data entered into the app:
    - questionnaire responses
    - timed walk/run test responses
    - manually-entered values for non-questionnaire dashboard tiles (eg blood pressure, LDL cholesterol, blood glucose)
  - SensorKit data:
...
- HealthKit data:
  - the app registers background observers for all HealthKit sample types it collects
  - the app then processes new samples for these by batch
    - batches always are homogeneous (ie all samples in a batch belong to the same sample type)
    - batches get processed into FHIR observations, which get JSON-encoded and zstd-compressed
  - each batch in the end becomes a `<filename>.json.zstd` file, which gets handed to the `ManagedFileUpload` module, for upload to the backend
  - format:
    - all HealthKit data (except `HKClinicalRecord`s) gets processed into [FHIR R4 Observation](https://hl7.org/fhir/R4/observation.html)s
      - the resulting resources get certain metadata attached:
        - the device time zone when uploading
        - the active study revision when uploading
        - (for samples originating from within MHC) the 
    - `HKClinicalRecord`s get encoded based on their underlying FHIR resource provided by HealthKit (either R4 or DSTU2, and any of AllergyIntolerance, Condition, Coverage, Immunization, MedicationOrder, MedicationRequest, MedicationStatement, MedicationDispense, Observation, Procedure, DiagnosticReport, or DocumentReference)
- SensorKit data:
  - the `SensorKitDataFetcher` module within the app is responsible for fetching SensorKit data
  - in contrast to HealthKit, SensorKit does not have a background observer/notify mechanism
  - the app instead processes SensorKit data on launch (in a background Task), plus it registers a `BGHealthResearchTask` to have iOS wake the app periodically in the background to fetch and process new SensorKit data
  - SensorKit data is processed as follows:
    - the `SensorKitDataFetcher` fetches new samples since the last fetch (anchor maintained by the SpeziSensorKit module)
    - all batches are homogeneous (each sample in a batch is coming from the same sensor)
    - each batch gets processed into a file, based on its underlying sensor:
      - JSON fila containing an array of FHIR observations: visits, onWrist, deviceUsage, ecg
      - CSV file containing a list of readings (1 CSV per batch): heartRate, pedometer, ambientLight, accelerometer, ambientPressure
      - CSV file containing a list of readings (1 CSV per sample): wristTemperature
      - custom binary data format: ppg
    - the JSON and CSV files get zstd-compressed before being uploaded to the backend
